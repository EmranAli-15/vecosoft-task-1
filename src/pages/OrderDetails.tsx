            ...
            <main className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
                ...
                <div className="card bg-base-200 border border-base-300 shadow-sm">
                    <div className="card-body p-5 sm:p-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 pt-6 border-t border-base-300 text-sm">
                            <div className="bg-base-100 p-4 rounded-xl border border-base-300">
                                <div className="font-bold opacity-70 text-xs uppercase mb-1">Destination Address</div>
                                <div className="font-medium">{currentOrder.shippingAddress}</div>
                            </div>
                            <div className="bg-base-100 p-4 rounded-xl border border-base-300">
                                <div className="font-bold opacity-70 text-xs uppercase mb-1">Carrier Details</div>
                                <div className="font-medium">{currentOrder.carrier}</div>
                                <div className="text-xs opacity-60 font-mono mt-1">Waybill: {currentOrder.trackingNumber}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="card bg-base-200 border border-base-300 shadow-sm">
                            <div className="card-body p-5 sm:p-6 space-y-3">
                                ...
                                <button
                                    onClick={() => setModalContent({
                                        title: 'Tracking Notice Unavailable',
                                        message: 'Live carrier telemetry is currently updating or experiencing high API latency. Your package is still moving through our logistics network according to schedule. Please check back in 2 hours.'
                                    })}
                                    className="btn btn-outline btn-sm w-full justify-start text-left border-base-300 hover:border-primary normal-case font-normal"
                                >
                                    ...
                                    Tracking not available right now?
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="card bg-base-200 border border-base-300 shadow-sm">
                    <div className="card-body p-5 sm:p-8">
                        <div className="max-w-xl">
                            <h2 className="text-lg font-bold tracking-tight">Contact Customer Support</h2>
                            <p className="text-sm opacity-70 mt-1">Have questions regarding order #{currentOrder.orderId}? Send us a direct inquiry below.</p>
                        </div>

                        <form onSubmit={handleSupportSubmit} className="mt-6 space-y-4 max-w-2xl">
                            <div className="form-control">
                                <label className="label">
                                    <span className="label-text font-medium text-xs uppercase opacity-70">Your Message or Issue</span>
                                </label>
                                <textarea
                                    className="textarea textarea-bordered h-24 focus:border-primary text-sm"
                                    placeholder="Describe your inquiry regarding items, delivery instructions, or address change..."
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    type="submit"
                                    className={`btn btn-primary ${supportSubmitted ? 'loading' : ''}`}
                                >
                                    {supportSubmitted ? 'Sending...' : 'Send Support Message'}
                                </button>
                                <span className="text-xs opacity-60">Average response time: &lt; 2 hours</span>
                            </div>
                        </form>
                    </div>
                </div>
